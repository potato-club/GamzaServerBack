...
public class JpaAssistance {
    ...
    public UserEntity getUserPkValue(Long id) {
        return userRepository.findById(id)
                .orElseThrow(() -> new UnAuthorizedException("잘못된 요청입니다 - 해당 유저가 존재하지 않습니다.", ErrorCode.BAD_REQUEST));
    }

    public ProjectEntity getProjectPkValue(Long id) {
        return projectRepository.findById(id)
                .orElseThrow(() -> new NotFoundException("잘못된 요청입니다 - 해당 유저가 존재하지 않습니다.", ErrorCode.BAD_REQUEST));
    }
